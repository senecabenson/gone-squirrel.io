...
## 2026-05-17 — Scheduling Blocks: Phase C shipped + live-verified (skip/reflow/makeup/move)

- **Branch:** `feat/scheduling-blocks` (NOT main, NOT pushed). Plan executed: `~/.claude/plans/continue-scheduling-blocks-feature-on-typed-honey.md`. Phase C = deterministic per-occurrence **skip** (cancel + free interval + temp work block + same-ISO-week makeup) and **move** (validate-first relocate), then re-run the existing `scheduleAllTasksForUser` (THAT is the reflow — no new scheduling logic; `SlotScorer`/energy untouched). Snooze dropped (overlaps Move — confirmed with user). Makeup = first-valid-slot, no `timesPerWeek` gate (ADHD: guaranteed re-placement > weekly accounting — user decision).
- **Did today (6 commits, strict TDD RED→GREEN per checkpoint, all pre-commit lint+tsc green):**
  - **CP0** — `skipReflowBlockType String @default("light")` was already in schema/types/store/fixture + already in Phase A migration `20260516184959`. Verified DB column exists; `prisma generate`. No migration, no commit.
  - **CP1 `04862d0`** — `CommitmentMaterializer.makeupOccurrence(commitmentId, isoWeek, excludeDateKey)` + `isoWeekBounds` (date-fns-tz, DST-correct). Reuses materialize's busy-set + `pickSlot` + planned→GCal→mirror 3-step. Skips the skipped day and days already holding an active CE for the commitment; no slot → `conflict`, never evicts. 6 unit tests.
  - **CP2 `6715cd1`** — NEW `CommitmentAdjuster.ts`: `skipOccurrence({reflow})` (per-occ cancel = single-row revoke; temp 🪶/🧠 block over freed interval tagged `gs:reflow:<id>`, gated by `skipReflowBlockType` + `reflow:"free"`; one makeup attempt) and `moveOccurrence(newStart)` **validate-first** (protected blocks via `matchBlockRule`, scheduled tasks, other active CEs, same-day double-book → `CommitmentMoveConflictError{code:"move_conflict"}` thrown BEFORE any mutation). 8 unit tests.
  - **CP2.1 `da4b504`** — **bug found while designing CP5:** `materialize`'s idempotency guard only short-circuited `materialized`, so a `cancelled` occurrence fell through to `pickSlot`+upsert and was **resurrected on the very recompute the skip route triggers** — silently undoing every skip. Added an explicit `cancelled` guard before the materialized guard. RED→GREEN regression test. (This was the make-or-break fix for Phase C.)
  - **CP3 `<feat/api>`** — NEW `/api/commitments/[id]/skip` & `/move` route handlers (existing `[id]` convention: `authenticateRequest` → ownership 404 → occurrence-belongs 404 → 400). skip → `skipOccurrence` → `scheduleAllTasksForUser`, returns `{skipped,reflow,makeup,plan}`. move → `moveOccurrence`; `CommitmentMoveConflictError` → 409 `{code:"move_conflict"}`, recompute short-circuited. Recompute lives in the **route** (not adjuster) — no scheduler import cycle. 8 handler tests (no route harness in repo → drive handlers directly w/ mocked deps + fake NextRequest).
  - **CP4 `0a90444`** — `PersonalCommitments.tsx`: per-future-occurrence **Skip** + inline-datetime **Move** + sonner toast surfacing makeup outcome / 409 ("collides with a protected block"); cancelled hidden; help note "edit raw Task Blocks in Google Calendar, next run reflows". No Snooze.
  - **CP5 `14083ed`** — live e2e `tests/e2e/commitments-adjust.spec.ts` + companion `scripts/e2e-reflow-cleanup.ts`. **1 passed live, EXIT=0, zero residue.** Proves on the REAL calendar: baseline parked → skip cancels (not resurrected) + temp 🪶 block + makeup same ISO week diff day → **displaced low-energy work REFLOWS INTO the freed interval (the B2-missing path, now proven)** → valid move + typed 409. Self-cleaning: revoke cascade + `e2e-reflow-cleanup.ts` (temp block isn't a CommitmentEvent so revoke can't reach it — dedicated cleanup) + settings restore.
- **Working:** Full `src/services/scheduling` suite **78/78**; route tests 8/8; live e2e 1/1. All 6 commits pass pre-commit. Branch NOT pushed (per user).
- **Hard-won CP5 lessons (future-me, do not relearn):**
  1. The materializer auto-places a fresh commitment at **~06:00 UTC** (`DAY_START_HOUR=6`, UTC arithmetic). In Seneca's negative-offset tz that's the local night — the **work scheduler will not fill it**, so a reflow proof there is dead. CP5 **moves** the occurrence to a DB-computed conflict-free **daytime UTC** slot first (move is also under test → doubles as a valid-move assertion).
  2. A pure-DB diagnostic (now deleted) proved the scheduler DOES place a low task into an eligible 🪶 feed block at a sane window — Phase A↔scheduler integration is sound; earlier nulls were the 06:00 tz artifact, not a product bug.
  3. The real Task Blocks feed is mirrored **far into the future** — hardcoded "empty" move targets collide with real `💖 Wife`/etc. blocks or scheduled tasks. `findConflictFreeStart(commitmentId,dur,minDay,maxDay)` computes targets against live data: **near** window (d 5–25, within work-scheduler horizon) for the reflow proof; **far** window (d 60–240, beyond the 14-day materialize horizon) so `moveOccurrence`'s same-day guard is satisfied by construction.
  4. e2e widens `AutoScheduleSettings` work hours→0–23, `bufferMinutes`→0, workDays→all (snapshotted + restored, incl. those fields) so the freed interval is reachable; baseline still parks because the interval is a **protected** commitment block (Phase A filter rejects regardless of work hours).
- **Next concrete task:** Phase C deterministic core is COMPLETE + live-verified. Remaining/optional: (a) NL adapter — deferred to the brain-dump LLM pipeline (no new cost surface) when that ships; maps "skip gym today"/"move golf to Thu" → the same `/skip`|`/move` endpoints. (b) Consider exposing `skipReflowBlockType` in the Auto-Schedule settings UI (currently default "light", no control; `src/store/settings.ts` `updateAutoScheduleSettings()` also omits the Phase-A/C block fields from its sync — pre-existing, out of Phase C scope). (c) Push branch / open PR when user asks (NOT done — user hasn't asked).

---

## 2026-05-17 — Phase C review remediation (R1–R5, pre-PR hardening)

- **Branch:** `feat/scheduling-blocks` (pushed). Plan: `~/.claude/plans/continue-scheduling-blocks-feature-on-typed-honey.md` (overwritten with the remediation plan). A multi-lens review produced 20 findings; each verified against real code (not blind-accepted). User approved the **Full FIX set**. 6 remediation commits, strict TDD, all pre-commit lint+tsc green.
- **Did today:**
  - **R1 `ce18143`** — CRIT security. (#1) `scripts/e2e-reflow-cleanup.ts` hard-exits on null calendar ctx (was: feedId filter dropped → `gs:reflow:`/`gs:e2e-seed` deletes matched ALL feeds/users in shared dev DB). (#2) PATCH `/api/commitments/[id]` now allowlists `{label,emoji,durationMin,rrule,preferredHour,timesPerWeek,active}` (was: raw `body`→`prisma.update({data})` = userId/relation mass-assignment IDOR; POST was already safe). +5 PATCH handler tests.
  - **R2 `784b07e`** — CRIT/HIGH idempotency. (#3) `skipOccurrence` early-returns on already-`cancelled` (was: double-tap re-deleted GCal, wrote a 2nd `gs:reflow` block, ran a duplicate makeup); routes reject up front (skip→409 `already_skipped`, move→409 `move_conflict` unless `materialized`). (#4) `makeupOccurrence` now adds cancelled days for the commitment to the skip set (was: could pick a separately-skipped day → upsert resurrected it — the CP2.1 bug, missing in makeup). +double-skip/route-409/cancelled-day tests; makeup mock harness filters string `status`.
  - **R3 `7d44148`** — HIGH. (#5) `expandOccurrences` anchored to `commitment.createdAt` not rolling `from` (was: INTERVAL>1 rules re-phased every run). (#7) `MAX_HORIZON_DAYS=90` backstop in `materialize` + clamp at the materialize route; `validateRrule()` (FREQ∈{DAILY,WEEKLY,MONTHLY}, COUNT≤366, INTERVAL≤52) enforced on POST+PATCH (400). +biweekly-phase / clamp / rrule-reject tests.
  - **R4 `8a28494`** — MED recovery. (#12) `moveOccurrence` snapshots `oldEventId`, nulls `googleEventId` in DB BEFORE the GCal delete (was: crash mid-move → stale googleEventId + materialized → idempotency guard skipped it forever). (#13) `makeupOccurrence` reverts the row to `conflict` on GCal-insert failure (was: left `planned` → its slot blocked others). +R4a/R4b tests + strengthened move-payload assertion (R5c) + year-boundary `isoWeekBounds` test (R5b).
  - **R5 `979e2bd`** — HIGH/LOW hardening. (#10) e2e settings wiring moved to `test.beforeAll` (was: throw-before-restore left the REAL user at widened workHours 0–23/7-day/buffer-0 permanently). (#19) `seed-e2e` refuses non-local `DATABASE_URL`. (#15) `TimeSlotManager` `parseBlockTypeMap(... ?? "[]")` defensive default.
- **Working:** Full `src/services/scheduling` + commitments API **103/103**; lint+tsc green every commit. Live e2e re-run post-remediation: see below.
- **Verified DISAGREE (recorded, not changed):** #6 not cross-user (`moveOccurrence` ctx = authed owner's own feed; title is owner UX). #9 false premise (`parseBlockTypeMap("[]")`→`DEFAULT_BLOCK_TYPE_MAP`; feature works on defaults — proven by passing live e2e; only customization UI absent). #15 parse already DEFAULTs on bad input (added `?? "[]"` only as cheap defensiveness).
