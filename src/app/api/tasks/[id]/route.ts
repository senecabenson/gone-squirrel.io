...
export async function PUT(
  ...
  try {
    ...
    const gcalRelevantChanged =
      Object.keys(updates).some((k) => gcalRelevantKeys.includes(k)) ||
      tagIds !== undefined;
    if (gcalRelevantChanged) {
      try {
        await syncSingleTaskToGoogle(userId, id);
      } catch (gcalError) {
        logger.error(
          "Single-task GCal sync failed (PUT response unaffected)",
          {
            taskId: id,
            error:
...
export async function DELETE(
  ...
  try {
    ...
    // Remove mirrored GCal event before deleting the task locally.
    if (task.googleEventId) {
      try {
        await deleteTaskGoogleEvent(userId, task.googleEventId);
      } catch (gcalError) {
        logger.error(
          "Failed to delete mirrored Google event during task DELETE",
          {
            taskId: id,
            error:
              gcalError instanceof Error
